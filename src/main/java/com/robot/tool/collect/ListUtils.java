package com.robot.tool.collect;



import com.robot.tool.reflect.ReflectUtils;
...
//	}
     ...
     */
    public static <T> void fillList(List<T> list, T t, Integer time) {
     ...
     */
    public static <T> List<List<T>> splitList(List<T> source, Integer size) {
        List<List<T>> res = new ArrayList<>(1 << 4);
        if(size == 0){
            res.add(source);
            return res;
        }else {
